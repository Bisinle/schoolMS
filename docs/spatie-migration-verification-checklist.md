...
**Coverage: 283/283 rows checked (275 pass, 0 fail, 8 N/A/inert). Exhaustive,
as requested, and every bug found along the way is now fixed** — every row
confirmed live: GET navigation for view-type permissions, a real
authenticated POST/PUT/DELETE (via curl with an extracted session cookie, or
an actual UI click-through where that proved more reliable — see the
tooling note below) for every state-changing permission, and console-error
checking on every positive-case page load, not just the HTTP status.
"Denied" always means a hard block confirmed the same way — never inferred
from a missing nav link.
...
## Real bugs found — 14 total across both passes, 14 fixed, 0 remaining
...
**5 more fixed (2026-08-29, one commit each — see the worksheet for the
exact commit hashes):**

| # | Permission | Was | Fix |
|---|---|---|---|
| 1 | `settings.manage` | `/settings/academic` 500'd — rendered a page component that never existed | `SchoolSettingController::academic()` now redirects to `settings.academic-years`, the real page every nav link already points to. The page's only actual data (`headteacher_signature`) had zero frontend consumers anywhere in the app — genuinely dead, not silently dropped functionality. |
| 2 | `timetable-slots.view` (index) | 500'd ordering by the nonexistent `period_id` column whenever a slot existed | `TimetableSlotController::index()` now orders by the real column, `timetable_period_id`. Confirmed no other `period_id` typos anywhere else in the file. |
| 3 | `documents.create` (admin) | 500'd for any school where an admin had ever used `guardians.delete` | `getEntityOptions()`'s admin branch now filters out guardians whose linked user is gone before mapping, instead of crashing on `$guardian->user->name`. Confirmed `TeacherController::destroy()` has no equivalent risk (hard-deletes both records together) before scoping the fix to Guardian only. |
| 4 | `documents.update` | `Documents/Edit.jsx` crashed on every document | Root-caused to be an entirely wrong file — a leftover copy of the Teachers Edit form, unrelated to Documents, expecting a `teacher` prop that was never passed. Wrote a correct, minimal Edit.jsx from scratch (single `expiry_date` field, matching `DocumentController::update()`'s actual validation and the sibling `Documents/Categories/Edit.jsx`'s established style). |
| 5 | `accident-reports.view` | `Show.jsx` crashed rendering `people_involved`/`witnesses` | Root-caused precisely: real UI-created reports only ever store plain scalar IDs (traced through Create.jsx → SearchableMultiSelect) — the crash only reproduced with the richer `{type, id, name}` object shape the migration's own comment documents as intended, which my original test fixture had used. Fixed defensively for both shapes with a `personLabel()` helper, since the object shape remains a legitimate possibility for this JSON column. |
...
| Permission | Role | Expected | Status | Notes |
|---|---|---|---|---|
| `settings.manage` | admin | Allowed | PASS | FIXED 2026-08-29: SchoolSettingController::academic() was rendering Settings/Academic.jsx, a page that never existed. That route had no nav link and no frontend consumer of its only real purpose (headteacher_signature, unused anywhere in the JS codebase) — every actual academic-settings nav link already points at the two dedicated, working pages (Settings/AcademicYears, Settings/AcademicTerms). Changed academic() to redirect to settings.academic-years instead of rendering a nonexistent page. Re-verified live: /settings/academic now 302s to /admin/settings/academic-years and renders real content. |
...
| `timetable-slots.view` | admin | Allowed | PASS | FIXED 2026-08-29: TimetableSlotController::index() ordered by the nonexistent `period_id` column (real column: `timetable_period_id`) — confirmed no other `period_id` references anywhere else in the file before fixing. Re-verified live with a real slot present (the bug only reproduced with at least one row, per the earlier MySQL empty-result-set finding): /timetables/slots now renders the slots table with no 500. |
...
| `documents.create` | admin | Allowed | PASS | FIXED 2026-08-29: getEntityOptions()'s admin branch now filters out guardians whose linked user is gone (`->filter(fn ($guardian) => $guardian->user !== null)`) before mapping, instead of crashing on `$guardian->user->name`. Confirmed TeacherController::destroy() has no equivalent risk (it hard-deletes both the Teacher and User together, so no orphaned Teacher row can exist — checked before scoping the fix to Guardian only, per the bug report). Re-verified live: reproduced the exact orphan condition (deleted a guardian's user, left the Guardian row), then confirmed /documents/create renders cleanly with no 500 and the entity picker no longer offers the orphaned guardian. |
...
| `documents.update` | admin | Allowed | PASS | FIXED 2026-08-29: Documents/Edit.jsx was not a stale-relation issue — root-caused it to be an entirely wrong file, a leftover copy of the Teachers Edit form (`export default function TeachersEdit({ teacher, grades, assignedGradeIds, classTeacherGradeId })`, confirmed via diff against Teachers/Edit.jsx), unrelated to Documents entirely. `teacher.user?.name` crashed because the real prop passed is `document`, not `teacher` — `teacher` itself was undefined (only `.user` was optional-chained, not `teacher` itself), matching the observed "Cannot read properties of undefined (reading user)" exactly. DocumentController::edit()/update() only ever deal with a single field (`expiry_date`, nullable, per the update() validation rule) — wrote a correct, minimal Edit.jsx from scratch matching the established style of the sibling Documents/Categories/Edit.jsx (same layout, same Tailwind classes, same Cancel/Save button pattern). Re-verified live: GET /documents/{id}/edit renders the real expiry-date form with zero console errors, and a real PUT with a new date persisted correctly (confirmed via DB). |
...
| Permission | Role | Expected | Status | Notes |
|---|---|---|---|---|
| `accident-reports.view` | admin | Allowed | PASS | FIXED 2026-08-29: root-caused precisely before fixing — traced through Create.jsx and SearchableMultiSelect and confirmed real, UI-created accident reports only ever store plain scalar person IDs in people_involved/witnesses (`onChange([...selected, optionId])`), never the `{type, id, name}` object shape the migration's own inline comment describes as intended. My original verification-pass fixture had used that documented object shape, which is what actually crashed — real admin-created reports were never reachable through this exact path. Fixed defensively anyway since the object shape is a legitimate, documented possibility for this JSON column: added a personLabel() helper that renders `.name` when the entry is an object and falls back to "Person ID: X" for a plain scalar, replacing the unconditional `{personId}` in both the People Involved and Witnesses blocks. Re-verified live with BOTH shapes: scalar IDs render "Person ID: 41" correctly (matching real-world data), and the original crash-triggering object shape now renders the person's actual name with zero console errors — a UX improvement over the scalar case, not just a non-crash. |
| `accident-reports.view` | teacher | Allowed | PASS | FIXED 2026-08-29 — see admin's row for the fix; teacher's access to this page was never the problem (already correctly permitted), only the render crashed, and that's now fixed for every role. |
| `accident-reports.view` | guardian | Allowed | PASS | FIXED 2026-08-29 — see admin's row for the fix. Reachable at 200 HTTP level (confirms Phase 2 disagreement #4 — no nav link but URL works), and the Show page now renders correctly with no crash. |
