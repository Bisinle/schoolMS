...
export default function Edit({ auth, users, students, report }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        people_involved: (report.people_involved || []).map(id => Number(id)), // Convert to numbers
        description: report.description || '',
        immediate_action_taken: report.immediate_action_taken || '',
        witnesses: (report.witnesses || []).map(id => Number(id)), // Convert to numbers
        medical_attention_required: report.medical_attention_required || false,
        medical_facility: report.medical_facility || '',
        medical_notes: report.medical_notes || '',
        parent_notified: report.parent_notified || false,
        parent_notified_at: report.parent_notified_at || '', // Already formatted by controller as YYYY-MM-DDTHH:MM
        parent_notification_method: report.parent_notification_method || '',
        follow_up_required: report.follow_up_required || false,
        follow_up_notes: report.follow_up_notes || '',
        follow_up_date: report.follow_up_date || '', // Already formatted by controller as YYYY-MM-DD
        _method: 'PUT',
    });
    ...
    const handleSubmit = (e) => {
        ...
        post(route('accident-reports.update', report.id), {
            data: formData,
            forceFormData: true,
        });
    };
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title="Edit Accident Report" />
            ...
            <div className="py-6">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="mb-6">
                        <button
                            onClick={() => router.visit(route('accident-reports.show', report.id))}
