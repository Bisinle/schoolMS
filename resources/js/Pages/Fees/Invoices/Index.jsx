...
import { Plus, Receipt, Eye, Trash2, DollarSign, Calendar, User, FileText } from 'lucide-react';
...
function MobileInvoiceItem({ invoice, auth, onDelete }) {
    const primaryActions = [
        { icon: Eye, label: 'View', href: `/invoices/${invoice.id}`, color: 'indigo' },
    ];

    if (auth.user.role === 'admin' && invoice.amount_paid === 0) {
        primaryActions.push(
            { icon: Trash2, label: 'Delete', onClick: () => onDelete(invoice), color: 'red' }
        );
    }
                ...
                header={
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between gap-2 mb-2">
                            ...
                            <Badge variant="invoiceStatus" value={invoice.status} size="sm" />
                        </div>
                        ...
                        <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-1.5 text-xs">
                                ...
                                <span className="font-semibold text-gray-900">KSh {invoice.total_amount.toLocaleString()}</span>
                            </div>
                            <div className="text-xs">
                                <span className="text-gray-600">Balance:</span>
                                <span className="font-semibold text-red-600 ml-1">KSh {invoice.balance_due.toLocaleString()}</span>
                            </div>
                        </div>
                    </div>
                }
                ...
                <div className="px-4 pb-4 pt-3 space-y-3">
                    <div className="grid grid-cols-1 gap-2">
                        ...
                        <div className="flex items-center gap-2.5 text-sm">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-xs text-gray-500 mb-0.5">Academic Term</p>
...
export default function InvoicesIndex({ auth, invoices, terms, filters: initialFilters }) {
    ...
    return (
        <AuthenticatedLayout header={auth.user.role === 'guardian' ? 'My Invoices' : 'Invoices'}>
            <Head title="Invoices" />
            ...
            <div className="space-y-6">
                ...
                {auth.user.role === 'admin' && (
                    <FilterBar>
                            ...
                            <FilterSelect
                                ...
                                onChange={(e) => updateFilter('term_id', e.target.value)}
                                options={[
                ...
                {invoices.data.length === 0 && (
                        <EmptyState
                            ...
                            message={auth.user.role === 'guardian'
                                                ...
                                                {auth.user.role !== 'guardian' && (
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="text-sm text-gray-900">{invoice.guardian?.user?.name}</span>
                                                        </div>
                                                    </td>
                                                )}
                                                <td className="px-6 py-4 whitespace-nowrap">
