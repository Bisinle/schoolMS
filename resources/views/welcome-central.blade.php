<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>School Management System</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gradient-to-br from-[#0b1a34] to-[#1e3a5f]">
    <div class="min-h-screen flex items-center justify-center px-4">
        <div class="text-center text-white max-w-2xl">
            <h1 class="text-5xl font-bold mb-4">School Management System</h1>
            <p class="text-xl mb-8 text-gray-300">
                Empowering Islamic schools with modern technology
            </p>
            <div class="space-x-4">
                <a href="https://admin.school-ms.com" 
                   class="inline-block bg-[#f97316] hover:bg-orange-600 text-white font-semibold px-8 py-3 rounded-lg transition-colors">
                    Admin Portal
                </a>
            </div>
            <p class="mt-12 text-sm text-gray-400">
                Schools access their subdomain: yourschool.school-ms.com
            </p>
        </div>
    </div>
</body>
</html>